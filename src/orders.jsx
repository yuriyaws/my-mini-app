   ...
   return(
        ...
        <h1 className="text-xl font-bold text-black">Заказы</h1>
            <div className="mt-3 bg-gray-100 rounded-lg px-2 py-1 flex flex-row gap-3">
                <button className={page === 'ordersActive' ? 'w-full py-1 bg-white rounded-lg text-sm' : 'w-full py-1 text-sm'}
                onClick={() => setPage('ordersActive')}
                >
                    Активные
                </button>
                <button className={page === 'ordersComplete' ? 'w-full py-1 bg-white rounded-lg text-sm' : 'w-full py-1 text-sm'}
        ...
        {/* Контент */}
        {page === 'ordersActive' && <OrdersActive />}
        {page === 'ordersComplete' && <OrdersComplete />}
        </>
    )
}
